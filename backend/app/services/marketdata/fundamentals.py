...
def _quote_from_fast_info(yf_obj: yf.Ticker) -> tuple[float | None, float | None, float | None]:
    """Returns (last_price, previous_close, market_cap). None means UNKNOWN.

    This used to return 0.0 as its failure sentinel, and get_overview() wrote
    that straight into `current_price` with no guard — which is how a
    rate-limited fetch became a confident "₹0.00" on the company page. There
    is no price of zero; a caller that sees a number has to be able to trust
    it, so unknown is now None and every consumer has to decide what to show.

    Delegates to yf_safe.read_quote so the fast_info -> history fallback and
    the zero/NaN rejection live in exactly one place.
    """
    return yf_safe.read_quote(yf_obj)
...
def get_overview(ticker: str) -> dict:
    ...
    # None all the way through when the quote is unknown, rather than 0.0 —
    # the frontend renders a null price as "—" and a zero as a real ₹0.00.
    # `change_percent` has to go None too: with no price there is nothing to
    # compare, and a 0.0 there reads as "flat today", which is a stronger
    # (and wronger) claim than "unknown".
    change = ((price - prev) / prev * 100) if (price is not None and prev) else None
    if price is None:
        logger.warning("overview for %s has no usable quote — serving price=None", ticker)
