...
from app.nse_universe import NSE_STOCKS, TICKER_TO_META, TICKER_TO_YF, resolve_yf_symbol
...
# Sentinel for "Yahoo answered about this symbol, but had no price for it".
# Distinct from None (nothing at all -> looks delisted) purely so the caller
# can pick the 60s negative TTL instead of the 24h one. See _inner below.
_NO_PRICE_ALIVE = object()


def _fetch_snapshot(ticker: str) -> dict:
    """Raw yfinance pull for one ticker. Returns {} on failure.

    NEVER returns a row without a real price — see _inner. A priceless
    snapshot used to be emitted with current_price 0.0, which every consumer
    read as a genuine price of zero.
    ...
    def _inner() -> dict | None:
        ...
        try:
            ...
            info = {}
        price, prev, mcap = yf_safe.read_quote(t)
        if mcap is None:
            mcap = yf_safe._num(info.get("marketCap"))

        # A PRICE IS MANDATORY. This used to be an `or` across price and the
        # fundamentals, which contradicted the comment right above it and was
        # the cause of the ₹0.00 tickers: Yahoo rate-limits its quote endpoint
        # far sooner than quote_summary, so on a shared IP you routinely get
        # no price but a cached marketCap — the `or` passed on that alone and
        # emitted current_price 0.0, which reads downstream as a real price of
        # zero rather than as missing data. Delisted symbols land here too:
        # Yahoo keeps serving their name/exchange metadata long after the
        # price is gone.
        #
        # Returning early (not a partial dict) is deliberate: the caller gets
        # {}, i.e. "no snapshot", which every consumer already handles. A
        # snapshot without a price cannot support a P&L, a 52-week position,
        # or a pre-trade check, so there is nothing it could usefully be used
        # for.
        #
        # WHICH failure it is decides the negative-cache TTL, and getting that
        # wrong is worse than the bug above: `None` means delisted and is
        # cached for 24h, so treating a throttled quote as delisted would
        # blank a perfectly good ticker for a whole day after one bad second.
        # Yahoo still serving fundamentals is proof the symbol is alive and
        # only the quote endpoint was throttled -> transient, retry in 60s.
        if price is None:
            alive = any(
                info.get(k) is not None
                for k in ("trailingPE", "returnOnEquity", "marketCap", "shortName")
            )
            return _NO_PRICE_ALIVE if alive else None
        return {
            "current_price": _safe(price),
            "previous_close": _safe(prev),
            "change_percent": _safe(((price - prev) / prev * 100) if prev else None),
            "market_cap": mcap,
    ...
    if result is _NO_PRICE_ALIVE:
        ...
        # so the next request retries rather than blanking it for 24h.
        _snapshot_neg_transient[ticker] = True
        logger.info("snapshot for %s: symbol alive but no quote — retrying in %ss",
                    ticker, yf_safe.NEG_TTL_TRANSIENT_S)
        return {}
    if result is None:
        _snapshot_neg_perm[ticker] = True
        return {}
