...
from app.services.pathback import accuracy_monitor, eval_runner
...
def _expected_direction(
    return_pct: float, horizon: str, sigma_daily_pct: float | None = None
) -> str:
    ...
    `sigma_daily_pct` must be the SAME value the grade used — it rides along
    on the row from outcome_ledger.graded_misses(). Passing None here when
    the grade used sigma silently reverts this to the flat band, which can
    name a direction the grader would score as a miss: the fixture would then
    train the drafter toward an answer the grader rejects. None is correct
    only for rows whose stored threshold_basis is 'flat'."""
    threshold = outcome_ledger.hit_threshold_pct(horizon, sigma_daily_pct)
    ...
    for m in misses[:limit]:
        ...
        try:
            ...
            expected = _expected_direction(
                return_pct, HORIZON, m.get("sigma_daily_pct")
            )
